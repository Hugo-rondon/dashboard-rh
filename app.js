...
function init() {
  ...
  $("turnoverMonthFilter").innerHTML = months.map(([value, label]) => option(value, label)).join("");
  $("turnoverMonthFilter").value = months.at(-1)[0];
  $("turnoverLotFilter").innerHTML = lotOptions;

  const thirdLots = [...new Set([...data.contractors.byLot, ...data.select.byLot].map(row => row.lot))]
  ...
  $("turnoverMonthFilter").addEventListener("change", renderTurnover);
  $("turnoverLotFilter").addEventListener("change", renderTurnover);
...
function renderWorkforce() {
  ...
  $("activeTotal").textContent = number(active);
  $("inactiveTotal").textContent = number(inactive);
...
function financeRows(month, lot = "all") {
  return data.finance.filter(row => row.month === month && (lot === "all" || row.lot === lot));
}

function turnoverRows(month, lot = "all") {
  return data.turnover.filter(row => row.month === month && (lot === "all" || row.lot === lot));
}

function renderFinance() {
  const month = $("monthFilter").value;
  const lot = $("financeLotFilter").value;
...
function renderTurnover() {
  const month = $("turnoverMonthFilter").value;
  const lot = $("turnoverLotFilter").value;
  const selected = turnoverRows(month, lot);
  const label = $("turnoverMonthFilter").selectedOptions[0].textContent;
  const averageHeadcount = sum(selected, "averageHeadcount");
  const dismissals = sum(selected, "dismissals");
  const admissions = sum(selected, "admissions");

  $("admissionMetric").textContent = number(admissions);
  ...
  const rowsHtml = data.lots.map(name => {
    const row = turnoverRows(month, name)[0] || {};
    const hidden = lot !== "all" && lot !== name;
    return `<tr${hidden ? ' style="opacity:.25"' : ""}><td>${name}</td><td>${number(row.startHeadcount)}</td><td>${number(row.endHeadcount)}</td><td>${number(row.admissions)}</td><td>${number(row.dismissals)}</td><td>${number(row.averageHeadcount)}</td><td><strong>${percent(row.turnoverRate)}</strong></td></tr>`;
  }).join("");

  const totalRate = averageHeadcount ? dismissals / averageHeadcount : 0;
  $("turnoverTable").innerHTML = `${rowsHtml}<tr><td><strong>Total</strong></td><td><strong>${number(sum(selected, "startHeadcount"))}</strong></td><td><strong>${number(sum(selected, "endHeadcount"))}</strong></td><td><strong>${number(admissions)}</strong></td><td><strong>${number(dismissals)}</strong></td><td><strong>${number(averageHeadcount)}</strong></td><td><strong>${percent(totalRate)}</strong></td></tr>`;
}
...
function renderThirdParties() {
  ...
  const contractorLots = filtered(data.contractors.byLot, lot);
  const companies = filtered(data.contractors.companies, lot);
  const selectLots = filtered(data.select.byLot, lot);
  ...
  $("selectPeople").textContent = number(sum(selectLots, "people"));
  $("selectCost").textContent = money(sum(selectLots, "cost"));
  renderBars("contractorBars", contractorLots);
  renderBars("selectBars", selectLots);
